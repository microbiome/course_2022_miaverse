...
```
## Speed comparison of beta diversity estimation
...
```{r}
# estimate beta diversity in tse
start.time1 <- Sys.time()
...
```{r}
# estimate beta diversity in pseq
start.time2 <- Sys.time()

GP.ord <- ordinate(pseq, "MDS", "bray")
p = plot_ordination(pseq, GP.ord, type = "samples", color = "SampleType")
...
```{r}
# sanity check
# plots are used as sanity check
```
